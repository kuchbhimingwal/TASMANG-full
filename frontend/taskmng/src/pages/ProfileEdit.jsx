...
function ProfileEdit() {
  ...
  const clickHandler = async()=>{
    ...
    try {
      const response = await axios.post('http://localhost:3001/user/userUpdate',updateObj,axiosConfig);
      try {
        const users = await axios.get('http://localhost:3001/user/getUsers', axiosConfig)
        const user = await axios.get('http://localhost:3001/user/getuser', axiosConfig)
