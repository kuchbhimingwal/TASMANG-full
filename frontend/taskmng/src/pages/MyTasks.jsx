...
function MyTasks() {
  ...
  const completedHandler= async(id)=>{
    ...
    try {
      const response = await axios.post('https://tasmang-backend.kuchbhimingwal.com/user/taskcomplete',bodyConfig,axiosConfig);
  ...
  useEffect(()=>{
    ...
    const dataFetchin=async()=>{
      ...
      try {
        const response = await axios.get(`https://tasmang-backend.kuchbhimingwal.com/user/tasksInUser`,axiosConfig)
